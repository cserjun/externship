...
//导入模块
  ...
  export default {
    ...
    mounted: function () {
     // this.getCategories();
      var from = this.$route.query.from;//获取from参数
      this.from = from;
      var _this = this;
    ...
    methods: {
      ...
      saveBlog(state){
        if (!(isNotNullORBlank(this.article.notification))) {
         //$message用于提示
        ...
        _this.loading = true;
        //调用封装的axios
        postRequest("/admin/addNotification", {
           id: _this.article.id,
           notification:_this.article.notification,
...
//            }
            ...
            // }
            if (state == 1) {
              _this.$router.replace({path: '/NotificationList'});
            }
          }
        }, resp=> {
          _this.loading = false;
          _this.$message({type: 'error', message: state == 0 ? '保存草稿失败!' : '公告发布失败!'});
        })
      },
