<template>
  <el-container>
    <el-header class="cate_mana_header">
      <el-input
        placeholder="请输入你的问题"
        v-model="question" class="que">
      </el-input>
      
      <el-button type="primary" size="medium" style="margin-left: 10px" @click="addNewCate">新增问题</el-button>
    ...
    <el-main class="cate_mana_main">
      ...
      <el-button type="danger" :disabled="this.selItems.length==0" style="margin-top: 10px;width: 100px;"
                 @click="deleteAll" v-if="this.questions.length>0">批量删除
      </el-button>
    </el-main>
  </el-container>
</template>
  ...
  export default{
    methods: {
      addNewCate(){
        this.loading = true;
        var _this = this;
        postRequest('/question/', {question: this.question}).then(resp=> {
          if (resp.status == 200) {
            var json = resp.data;
      ...
      handleDelete(index, row){
        let _this = this;
        this.$confirm('确认删除 ' + row.question + ' ?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
      ...
      deleteCate(ids){
        ...
        //删除
        deleteRequest("/question/" + ids).then(resp=> {
          var json = resp.data;
          _this.$message({
            type: json.status,
            message: json.msg
